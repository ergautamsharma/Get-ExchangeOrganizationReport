...
# Asking for Office 365 Credential
#$O365Cred = Get-Credential -Message "Provide Office 365 Global Admin Credential (MFA Disabled)"
...
function New-MicrosoftWordDocument {
  ...
  elseif($Script:WordVersion -eq $wdWord2007)	{
    ...
    $ErrorActionPreference = $SavedErrerActionPreference
    Write-Error -Message "You are running an untested or unsupported version of Microsoft Word.`nScript will end.`nPlease send info on your version of Word to Gautam.Sharma@hcl.com"
    Stop-Script
  }
  ...
  [bool]$Script:CoverPagesExist = $True
  [bool]$BuildingBlocksExist = $True
...
function Get-ExchangeOrganizationConfig {
  ...
  $PublicFolderInformation += @{ Data = 'Mailbox Folder Delegated Permissions'; Value = $OrgConfig.ACLableSyncedObjectEnabled; }
...
function Get-TransportConfigInformation {
  ...
  $ObjectInformation += @{ Data = 'Hygiene Suite'; Value = $TransportConfig.HygieneSuite; }
  $ObjectInformation += @{ Data = 'External Postmaster Address'; Value = $TransportConfig.ExternalPostmasterAddress; }
  $ObjectInformation += @{ Data = 'Max Receive Size'; Value = $TransportConfig.MaxReceiveSize; }
  $ObjectInformation += @{ Data = 'Max Send Size'; Value = $TransportConfig.MaxSendSize; }
  $ObjectInformation += @{ Data = 'External Message Attach Size'; Value = $TransportConfig.ExternalDsnMaxMessageAttachSize; }
  $ObjectInformation += @{ Data = 'Internal Message Attach Size'; Value = $TransportConfig.InternalDsnMaxMessageAttachSize; }
  $ObjectInformation += @{ Data = 'Max Dumpster Size Per Database'; Value = $TransportConfig.MaxDumpsterSizePerDatabase; }
  $ObjectInformation += @{ Data = 'Max Dumpster Time'; Value = $TransportConfig.MaxDumpsterTime; }
  
  #Journaling
  $ObjectInformation += @{ Data = 'Journal Archiving Enabled'; Value = $TransportConfig.JournalArchivingEnabled; }
  $ObjectInformation += @{ Data = 'Legacy Journal Migrations Enabled'; Value = $TransportConfig.LegacyJournalingMigrationEnabled; }
  $ObjectInformation += @{ Data = 'Voicemail Journalingm Enabled'; Value = $TransportConfig.VoicemailJournalingEnabled; }
  $ObjectInformation += @{ Data = 'Journaling Report Ndr Send To'; Value = $TransportConfig.JournalingReportNdrTo; }
...
function Get-DatabaseOverviewInformation {
  ...
  $Text = 'Following are the overall Mailbox Database informations.'
...
function Get-MobileDeviceInformation {
  ...
  $MobileDevices = Get-MobileDevice -Resultsize Unlimited -ErrorAction SilentlyContinue

  $MobileDeviceCount = ($MobileDevices | Measure-Object).Count
  if ($MobileDeviceCount -ne $Null)
  {
  ...
  $Text = ('The Exchange Organization contains {0} journal rules.' -f $MobileDeviceCount)
...
function Get-SharingInformation {
  ...
  [Collections.Hashtable[]]$SPolicyHashTable = @()
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $SPolicyHashTable `
...
function Get-AppInformation {
  ...
  [Collections.Hashtable[]]$AppHashTable = @()
  ...
  $SectionTitle = 'Outlook Add-ins or Apps'
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $AppHashTable `
...
function Get-AddressListInformation {
  ...
  [Collections.Hashtable[]]$ALHashTable = @()
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $ALHashTable `
...
function Get-ReceiveConnectorInformation {
  ...
  [Collections.Hashtable[]]$RCHashTable = @()
  ...
  foreach ($Connector in $ReceiveConnectors) {
    ...
    $RCHashTable += @{ 
      ...
      AllowedIPs = ($Connector.RemoteIPRanges -join ', ');
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $RCHashTable `
    -Columns Server, Name, Bindings, AllowedIPs, Enabled `
    -Headers 'Server', 'Name', 'Bindings', 'AllowedIPs', 'Enabled' `
    -AutoFit $wdAutoFitFixed
...
function Get-PublicFolderMailboxInformation {
  ...
  [Collections.Hashtable[]]$PFHashTable = @()
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $PFHashTable `
...
function Get-DiskSpace {
  ...
  [Collections.Hashtable[]]$DSKHashTable = @()
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $DSKHashTable `
...
function Get-ExchangeServerInformation {
  ...
  [Collections.Hashtable[]]$EXHashTable = @()
  ...
  if($ExportTo -eq 'MSWord') {
    ...
    $Table = Add-WordTable -Hashtable $EXHashTable `
...
function Get-EXOConfigurationInformation {

  Show-ProgressBar -Status 'Office 365 - EXO Configuration' -PercentComplete 15 -Stage 1

  Connect-ExchangeOnline -Credential $O365Cred

  $EXOConfiguration = Get-OrganizationConfig

  if($HybridConfiguration -ne $null) {

    [Collections.Hashtable[]]$ObjectInformation = @()

    $ObjectInformation = Expand-Object -Object $EXOConfiguration
    ...
    Write-WordLine -Style 0 -Tabs 0 -Name 'This Exchange Organization does not have a Office 365 Exchange.'
  }
}
...
function Get-WordDocumentationLinks {
  ...
  $HashTableDocumentationLinks += @{Title='Microsoft Exchange';Link = 'https://docs.microsoft.com/en-us/exchange/'}
...
if($ExportTo -eq 'MSWord') {
  ...
  [long]$wdColorGray15 = 16764057 #changed the color to Pale blue
...
$SubjectTitle = 'Microsoft Exchange Inventory Report'
...
switch ($ExportTo) {
  'MSWord' {
    ...
    Set-SectionTitle -Style 1 -SectionTitle 'Exchange Organization' -NewPage
    ...
    #Get-EXOConfigurationInformation
